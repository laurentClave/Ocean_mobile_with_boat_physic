    Shader "Mobile/Ocean" {
        Properties {
            ...
            _Specularity ("Specularity", Range(0.03,1.0)) = 0.15
        ...
        SubShader {
            ...
            Pass {
                ...
                half4 frag (v2f i) : COLOR {
                    ...
                    half3 tangentNormal1 = (tex2D(_Bump, buv.xy) * 2.0) - 1;
                    half3 tangentNormal = normalize(tangentNormal0 + tangentNormal1);
     
                    half3 tangentNormalA0 = (tex2D(_FoamBump, i.bumpTexCoord.xy) * 2.0) - 1;
                    half3 tangentNormalA1 = (tex2D(_FoamBump, i.bumpTexCoord.xy) * 2.0) - 1;
                    half3 tangentNormalA = normalize(tangentNormalA0 + tangentNormalA1);
     
                    half3 normalsCombined = (tangentNormal +tangentNormalA)*0.5;
     
                    float2 projTexCoord = 0.5 * i.projTexCoord.xy * float2(1, _ProjectionParams.x) / i.projTexCoord.w + float2(0.5, 0.5);
                    half4 result = half4(0, 0, 0, 1);
                    float2 bumpSampleOffset = i.objSpaceNormal.xz * 0.05 + tangentNormal.xy * 0.05 +  tangentNormalA.xy * 0.05;
                    ...
                    float fresnelLookup = dot(normalsCombined, normViewDir);
                    ...
                    float specular = pow(max(dot(halfVec,  normalsCombined) , 0.0), 250.0 * _Specularity );
